...
class TestTransformer:
    def test_unit_normalization(self):
        row = {"patient_id": "P001", "lab_name": "Glucose", "value": "95", "unit": "MG/DL"}
        result = transform_row(row, _make_config())
        assert result["unit"] == "mg/dL"

    def test_custom_rule_replace(self):
        row = {"patient_id": "P001", "lab_name": "Na+", "value": "140", "unit": "mEq/L"}
        result = transform_row(row, _make_config())
    ...
    def test_value_normalization_integer(self):
        row = {"patient_id": "P001", "lab_name": "WBC", "value": "7.0000", "unit": "K/mcL"}
        result = transform_row(row, _make_config())
        assert result["value"] == "7"

    def test_value_normalization_float(self):
        row = {"patient_id": "P001", "lab_name": "Hgb", "value": "14.5000", "unit": "g/dL"}
        result = transform_row(row, _make_config())
        assert result["value"] == "14.5"

    def test_empty_value(self):
        row = {"patient_id": "P001", "lab_name": "Test", "value": "", "unit": "mg/dL"}
        result = transform_row(row, _make_config())
        assert result["value"] == ""
